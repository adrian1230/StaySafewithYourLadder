...
def generate(folder, img, objects, tl, br, savedir):
	...
	save_path = os.path.join(savedir, folder.split('/')[1],img.name.replace('jpg', 'xml')) #use image name
	with open(save_path, 'wb') as temp_xml:
		temp_xml.write(xml_str)

if __name__ == '__main__':
    folder = 'data'
    img = [im for im in os.scandir('data') if '2.jpg' in im.name][0]
    objects = ['Ladder', 'Hand', 'Feet']
    tl = [(10,10), (20,20)]
    br = [(100,100), (200, 200)]
    savedir = 'annotation'
